...
def read_json_file(path):
    ...
    """
    json_file = read_file(_get_full_path(path))
...
def open_file(path, open_type='w', overwrite=False):
    ...
    # Open an append file
        ...
        if does_file_exist(path):
            ...
            raise OSError(_build_error_str(2).format(arg=path))
    elif open_type == 'r' or open_type == 'r+':
        if does_file_exist(path):
            fd = open(path, open_type)
        else:
            raise OSError(_build_error_str(2).format(arg=path))
...
def write_file(path, to_write, append=False, overwrite=False):
    ...
    * to_write can be a str - preformatted and ready to go
    ...
    if isinstance(to_write, list) and isinstance(to_write[0], (str, unicode)):
        to_write = "\n".join(to_write)
...
def _get_full_path(path, *p):
    ...
    """
    return os.path.expanduser(os.path.join(path, *p))
